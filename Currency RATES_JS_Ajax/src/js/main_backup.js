...
window.onload = function() {
    if (document.forms["form-select"]) {
        ...
        formListen(form, 14);	//form in DOM and max time period in days
    }
};
 ...
 * */
function formListen(form, maxPeriod) {
                    ...
                    for (let i = 0; i < ratesArr.length; i++) {
                        if ("rates" in ratesArr[i]) {
                            if ("date" in ratesArr[i]) {
                                ...
                                graphResultColValue.textContent = ratesArr[i]["rates"][curEval]; //writing value to graph
